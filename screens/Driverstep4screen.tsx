...
export default function DriverStep4Screen() {
  ...
  const params = useLocalSearchParams(); // ← receives all params from steps 1, 2 & 3
  ...
  const handleNext = () => {
    ...
    // Pass ALL params forward to step 5
    // ...params keeps everything collected from all previous steps
    router.push({
      pathname: '/driver-step-5',
      params: {
        ...params,                  // ← everything from steps 1, 2, 3
        registrationImage,
        vehicleImage,
      },
    });
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Stack.Screen options={{ headerShown: false }} />  {/* ← fixed position */}
...
const styles = StyleSheet.create({
  ...
  sectionLabel: { fontSize: 16, color: '#4A5568', fontWeight: '600', marginBottom: 6 },
  sectionSubtext: { fontSize: 14, color: '#718096', marginBottom: 12, lineHeight: 20 },
  uploadCard: {
  ...
  uploadText: { fontSize: 16, fontWeight: '600', color: '#2A66B5', marginBottom: 8 },
  uploadSubtext: { fontSize: 14, color: '#718096', textAlign: 'center' },
  ...
  changePhotoText: { fontSize: 14, color: '#4D9EFF', fontWeight: '600' },
  bottomNav: {
    position: 'absolute',
    bottom: 0,
    left: 0,
