...
export default function DriverStep3Screen() {
  ...
  const params = useLocalSearchParams(); // ← receives all params from steps 1 & 2
  ...
  const incrementSeating = () => {
    if (seatingCapacity < 8) setSeatingCapacity(seatingCapacity + 1);
  };

  const decrementSeating = () => {
    if (seatingCapacity > 1) setSeatingCapacity(seatingCapacity - 1);
  };
  ...
  const handleNext = () => {
    ...
    // Pass ALL params forward to step 4
    // ...params keeps vehicle_type, firstName, lastName, licenseImage, plateNumber etc.
    router.push({
      pathname: '/driver-step-4',
      params: {
        ...params,                              // ← everything from previous steps
        vehicleModel,
        vehicleColor,
        vehicleYear,
...
const styles = StyleSheet.create({
  ...
  counterButtonDisabled: { backgroundColor: '#F7FAFC' },
  counterDisplay: { flex: 1, alignItems: 'center' },
  counterValue: { fontSize: 32, fontWeight: 'bold', color: '#2A66B5' },
  counterLabel: { fontSize: 14, color: '#718096', marginTop: 4 },
  bottomNav: {
    position: 'absolute',
    bottom: 0,
    left: 0,
