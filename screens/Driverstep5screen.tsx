...
import { addVehicle } from '../api'; // ← import api function

export default function DriverStep5Screen() {
  const router = useRouter();
  const params = useLocalSearchParams(); // ← receives ALL data from steps 1-4
  ...
  const handleFinish = async () => {
    ...
    if (!agreedToTerms) {
      Alert.alert(
        'Agreement Required',
        'Please agree to the Community Guidelines and Fair-Pricing policy to continue.'
      );
      return;
    }
    ...
    try {
      // ── STEP 2: Save vehicle to backend ───────────────
      // All data collected across steps 1-4 is available in params
      const vehicleData = {
        vehicle_type: params.vehicle_type as string,    // from ChooseVehicle
        make: params.firstName as string,               // we use make field for now
        model: params.vehicleModel as string,           // from step 3
        year: parseInt(params.vehicleYear as string),   // from step 3
        color: params.vehicleColor as string,           // from step 3
        license_plate: params.plateNumber as string,    // from step 2
        seats: parseInt(params.seatingCapacity as string), // from step 3
      };
      ...
      // ── STEP 4: Navigate to driver main after 2.5s ────
      setTimeout(() => {
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <View style={styles.bottomNav}>
          ...
          >
            {loading ? (
              <ActivityIndicator color="#FFFFFF" />  // ← shows spinner while saving
            ) : (
              <>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <Text style={styles.successMessage}>
              Thank you for completing your driver registration! Our team will
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: 18, fontWeight: '600', color: '#2A66B5', marginBottom: 6 },
  sectionSubtext: { fontSize: 14, color: '#718096', marginBottom: 16, lineHeight: 20 },
  cnicContainer: { flexDirection: 'row', gap: 12 },
  cnicCard: {
    flex: 1,
    aspectRatio: 1.6,
  ...
  cnicPlaceholder: { flex: 1, justifyContent: 'center', alignItems: 'center', padding: 12 },
  cnicText: { fontSize: 12, fontWeight: '600', color: '#4D9EFF', marginTop: 8 },
  cnicImage: { width: '100%', height: '100%' },
  preferencesCard: {
    backgroundColor: '#FFFFFF',
    borderRadius: 16,
    padding: 16,
  ...
  preferenceLeft: { flexDirection: 'row', alignItems: 'center', flex: 1, gap: 12 },
  preferenceIconContainer: {
    width: 40, height: 40, borderRadius: 10,
    backgroundColor: '#F0F7FF', justifyContent: 'center', alignItems: 'center',
  },
  preferenceLabel: { fontSize: 16, fontWeight: '600', color: '#2A66B5' },
  preferenceDescription: { fontSize: 13, color: '#718096', marginTop: 2 },
  ...
  checkboxChecked: { backgroundColor: '#4D9EFF', borderColor: '#4D9EFF' },
  agreementText: { flex: 1, fontSize: 14, color: '#4A5568', lineHeight: 20 },
  agreementLink: { color: '#4D9EFF', fontWeight: '600' },
  bottomNav: {
    position: 'absolute', bottom: 0, left: 0, right: 0,
    flexDirection: 'row', justifyContent: 'space-between',
    paddingHorizontal: 20, paddingVertical: 20,
    backgroundColor: '#F0F7FF', borderTopWidth: 1, borderTopColor: '#E2E8F0',
  },
  prevButton: {
  ...
  finishButton: {
    flex: 1, marginLeft: 16, height: 56, borderRadius: 12,
    backgroundColor: '#4D9EFF', flexDirection: 'row',
    justifyContent: 'center', alignItems: 'center', gap: 8,
    shadowColor: '#000', shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.15, shadowRadius: 8, elevation: 4,
  },
  finishButtonText: { fontSize: 18, fontWeight: '700', color: 'white', letterSpacing: 0.5 },
  modalOverlay: {
  ...
  successTitle: { fontSize: 24, fontWeight: 'bold', color: '#2A66B5', marginBottom: 12, textAlign: 'center' },
  successMessage: { fontSize: 16, color: '#4A5568', textAlign: 'center', lineHeight: 24, marginBottom: 12 },
  successSubtext: { fontSize: 14, color: '#718096', textAlign: 'center', lineHeight: 20 },
});
